...
-- =======================================================
-- THE FLAWLESS, GUARANTEED RESTORATION ARCHITECTURE 
-- =======================================================
...
CREATE UNIQUE INDEX unique_active_subscription_per_customer
  ON subscriptions(customer_id) WHERE status::text ILIKE 'active';
...
BEGIN
  ...
  WHERE s.status IN ('Active', 'Grace')
    AND (s.status::text ILIKE 'grace' OR s.remaining_tiffins > 0)
...
BEGIN
  ...
  updated AS (
    ...
    SET remaining_tiffins = s.remaining_tiffins - 1,
        status = CASE WHEN s.remaining_tiffins - 1 = 0 THEN 'Expired' ELSE s.status END,
...
BEGIN
  ...
  IF p_action = 'DEDUCT' THEN
    ...
    UPDATE subscriptions SET
      ...
      status = CASE WHEN remaining_tiffins - 1 = 0 THEN 'Expired' ELSE status END,
  ...
  ELSIF p_action = 'RESTORE' THEN
    ...
    UPDATE subscriptions SET
      remaining_tiffins = remaining_tiffins + 1,
      status::text ILIKE 'active'::subscription_status,
      completed_at = NULL
    WHERE id = p_sub_id;
...
SELECT
  ...
  COUNT(s.id) FILTER (WHERE s.status::text ILIKE 'active') AS active_count,
  COUNT(s.id) FILTER (WHERE s.status::text ILIKE 'completed') AS completed_count,
  COUNT(s.id) FILTER (WHERE s.status::text ILIKE 'cancelled') AS cancelled_count,
